class BoardService {
    ...
    async postWrite({ userid, subject , content }) {
      console.log(`serv :`, { userid, subject ,content });
      try {
        if (!userid || !subject || !content) throw "내용이 없습니다";
        const write = await this.boardRepository.createBoard({ userid, subject, content });
        return write;
      } catch (e) {
        throw new this.HttpException(e);
      }
    ...
    async postComment(boardid, userid, content) {
      ...
      async deleteLike(likeid, userid) {
        // console.log(`serv :`, { boardid, userid, content });
        try {
          if (!likeid || !userid) throw "추천 취소 실패";
          const remove = await this.boardRepository.destroyLike({ likeid, userid });
          return remove;
        } catch (e) {
          throw new this.HttpException(e);
        }
